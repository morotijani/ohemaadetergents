...
<div class="auth-shell">
  ...
  <div class="auth-form-side">
    <div class="auth-box" style="text-align:center;">
      ...
      <div id="resendAlert"
        style="padding: 15px; margin-bottom: 20px; font-size: 0.9rem; display: none; text-align: left;"></div>
      <form id="resendForm">
        <div class="field" style="text-align:left;">
          <label for="verifyEmail">Wrong email? Update it below</label>
          <input id="verifyEmail" name="email" type="email" placeholder="you@email.com"
            value="<?php echo htmlspecialchars($email); ?>">
        </div>
        <button class="form-submit btn-full" type="submit">Resend verification email</button>
      </form>

      <p class="resend-timer"></p>

      <p class="auth-foot" style="margin-top:28px;">Already verified? <a href="login">Log in</a></p>
      <p class="auth-foot">Wrong account? <a href="login">Sign out and switch</a></p>
    </div>
  </div>
</div>
...
<script>
  document.getElementById('resendForm').addEventListener('submit', async function (e) {
    ...
    // We removed the finally block because the success path handles re-enabling after the countdown
  });
</script>
